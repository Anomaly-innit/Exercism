...
def round_scores(student_scores):
    ...
    for score in student_scores:
        round_score.append(round(score))
    return round_score
...
def letter_grades(highest):
    ...
    all_scores = [41, 41+difference, 41+difference*2, 41+difference*3]
    return all_scores
...
def student_ranking(student_scores, student_names):
    ...
    for index, word in enumerate(student_names):
        result.append(f"{index+1}. {word}: {student_scores[index]}")
    return result
