...
def check_drinks(drink_name, drink_ingredients):
    ...
    if set(drink_ingredients).isdisjoint(ALCOHOLS):
        result = ( drink_name + " Mocktail")  
    else:
        result = ( drink_name + " Cocktail") 
    return result
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    sets = (VEGAN, VEGETARIAN, KETO, PALEO, OMNIVORE)

    for i, category_set in enumerate(sets):
        if dish_ingredients.issubset(category_set):
            return dish_name + ": " + categories[i]
...
def tag_special_ingredients(dish):
    ...
    """
    ingredients = dish[1]
    result = (dish[0], SPECIAL_INGREDIENTS.intersection(ingredients))
    return result
...
def singleton_ingredients(dishes, intersection):
    ...
    for i in all_ingredients:
        count = sum(1 for dish in dishes if i in dish)
        if count == 1 :
            result.add(i)
        count = 0
    return result
